...
public class SLSConfig implements TagUtil.Taggable {
  ...
  String securityToken = ""; // securityToken can be empty
  String topic;
  ...
  public LogProducerConfig getProducerConfig() {
    LogProducerConfig config = null;
    try {
      config = new LogProducerConfig(
        AppCtxUtil.getAppContext(),
        this.endpoint,
        this.project,
        this.logStore,
        this.accessKeyId,
      ...
      // !!!!!!!!!!!!!!!!!!!注意!!!!!!!!!!!!!!!!!!!
      // 配置多个客户端时，不应设置相同文件
      config.setPersistentFilePath(AppCtxUtil.getFilesDir() + String.format("%slog_data.dat", File.separator));
