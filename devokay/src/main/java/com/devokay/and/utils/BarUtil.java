...
public final class BarUtil {
  ...
  public static int getActionBarHeight() {
    ...
    if (U.getApp().getTheme().resolveAttribute(android.R.attr.actionBarSize, tv, true)) {
      return TypedValue.complexToDimensionPixelSize(
        tv.data, U.getApp().getResources().getDisplayMetrics()
      );
    }
    return 0;
  }
  ...
  private static void invokePanels(final String methodName) {
    try {
      ...
      Object service = U.getApp().getSystemService("statusbar");
  ...
  public static void setNavBarVisibility(@NonNull final Window window, boolean isVisible) {
    ...
    for (int i = 0, count = decorView.getChildCount(); i < count; i++) {
      ...
      if (id != View.NO_ID) {
        String resourceEntryName = U.getApp()
          .getResources()
